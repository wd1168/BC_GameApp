...
$title = $_GET['link'];
$game_query =
    "SELECT g.Game_ID, g.Name, i.Image AS 'Image', Description, Age, Player_Count, m.Name AS 'Manufacturer' FROM
...
while ($row = $statement2->fetch(PDO::FETCH_ASSOC)) {
    $game = array('Game_ID' => $row['Game_ID'], 'Name' => $row['Name'], 'Image' => 'data:image/jpeg;base64,' . base64_encode($row['Image']), 'Description' => $row['Description'],
        'Age' => $row['Age'], 'Player_Count' => $row['Player_Count'], 'Manufacturer' => $row['Manufacturer']);
}
$smarty->assign('game', $game);
// adds questions and response code
include("questions.php");
//Expansion Code- below
$exp_query = "SELECT Expansion_ID, e.Name AS 'Name' , g.Name AS 'G_Name', i.Image AS 'Image' FROM
...
while($xrow = $exp_statement->fetch(PDO::FETCH_ASSOC)){
    ...
    if ($old_exp != $xrow['Game_ID']) {
        ...
        $expansion = array (
            ...
            "Name" => $xrow['Name'],
            "G_Name" => $xrow['G_Name'],
            "Image" => 'data:image/jpeg;base64,' . base64_encode($xrow['Image'])
        );
